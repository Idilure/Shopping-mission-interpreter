...
def tag(label, style="gray"):
    return f'<span class="tag tag-{style}">{escape(str(label))}</span>'
...
def html_text(value):
    """Escape model and user text before placing it inside raw HTML."""
...
def render_result(result, label):
    ...
    if result.get("mission_summary"):
        st.markdown(
            f'<div class="mission-summary">"{html_text(result["mission_summary"])}"</div>',
            unsafe_allow_html=True)

    # top metrics
    occ = html_text(result["meal_occasion"])
    mis = html_text(result["mission_type"].replace("_"," "))
    bud = html_text(result["budget_sensitivity"].replace("_"," "))
    st.markdown(f"""
    <div class="metric-row">
      ...
      <div class="metric-box">
        <div class="val">{mis}</div>
        <div class="lbl">Mission Type</div>
      </div>
      <div class="metric-box">
        <div class="val">{bud}</div>
    ...
    if cats:
        ...
        if subcats:
            subcat_text = " · ".join(html_text(s) for s in subcats[:12])
            st.markdown(
                f'<div class="subcat">↳ {subcat_text}</div>',
                unsafe_allow_html=True)
    else:
        st.markdown(
    ...
    if result.get("explanation"):
        st.markdown(
            f'<div class="expl-box">💬 {html_text(result["explanation"])}</div>',
            unsafe_allow_html=True)
...
if "shopping_request" not in st.session_state:
    st.session_state.shopping_request = ""
...
def use_example(example):
    st.session_state.shopping_request = example
        ...
        if recs:
            ...
            for i, rec in enumerate(recs):
                with cols[i % 4]:
                    ...
                    filled = "█" * score
                    empty  = "░" * (10 - score)
                    product_name = html_text(rec["product"])
                    reason = html_text(rec["reason"])
                    aisle = html_text(rec["aisle"].replace('_',' '))
                    st.markdown(f"""
