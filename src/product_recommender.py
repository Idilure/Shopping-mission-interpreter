...
Return ONLY a JSON array of exactly {n} objects, each with:
  ...
  "match_score" : integer 1-10 scoring how well this product fits, based on:
                  ...
                  Be honest: score 9-10 only for near-perfect fits,
                  7-8 for good fits, 5-6 for partial, below 5 rarely.
...
def recommend_products(
    ...
    try:
        ...
        # the model should return {"recommendations": [...]} or just [...]
        if isinstance(raw, list):
            recs = raw
        elif isinstance(raw, dict):
            # find the first list value
            recs = next((v for v in raw.values() if isinstance(v, list)), [])
        else:
            return []
